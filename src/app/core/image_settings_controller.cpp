    ...
    QObject(parent)
{
    ...
    connect(phiEditor_.get(),
            &PhiEditor::phiAccepted,
            this,
            &ImageSettingsController::setInitialPhi);

    connect(phiViewModel_.get(),
...
ShapeInfo ImageSettingsController::computeShapeInfo(const UiShapeInfo& uiShape)
{
    ...
    float centerXPercent = static_cast<float>(uiShape.x) / 100.0f; // De -500 à +500, donc normalisé autour de 0
    ...
    float width  = static_cast<float>(uiShape.width)
                  / 100.0f * static_cast<float>(canvasWidth);

    float height = static_cast<float>(uiShape.height)
